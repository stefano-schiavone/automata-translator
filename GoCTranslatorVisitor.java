...
public class GoCTranslatorVisitor extends translatorBaseVisitor<String> {
    ...
    private String inferType(translatorParser.ExpressionContext ctx) {
        if (ctx instanceof translatorParser.IntExprContext)    return "int";
        if (ctx instanceof translatorParser.FloatExprContext)  return "double";
        if (ctx instanceof translatorParser.StringExprContext) return "char*";
        if (ctx instanceof translatorParser.BoolExprContext)   return "int";
    ...
    /** Scan all function declarations to build funcReturnTypes and generate struct preambles. */
    private void collectSignatures(translatorParser.ProgramContext ctx) {
        for (translatorParser.TopLevelDeclContext decl : ctx.topLevelDecl()) {
            ...
            if (fd.returnSignature() == null) {
                ...
                translatorParser.SingleReturnContext sr =
                    (translatorParser.SingleReturnContext) fd.returnSignature();
                ret.add(goToCType(sr.type_().getText()));
            } else {
                translatorParser.MultiReturnContext mr =
                    (translatorParser.MultiReturnContext) fd.returnSignature();
    ...
    public String visitFunctionDecl(translatorParser.FunctionDeclContext ctx) {
        ...
        String cReturn;
        if (name.equals("main"))       cReturn = "int";
        else if (ret.size() > 1)       cReturn = name + "_result";
        else                           cReturn = ret.get(0);
    ...
    private String visitShortVarDeclLines(translatorParser.ShortVarDeclContext ctx) {
        ...
        for (var id : ctx.IDENTIFIER()) names.add(id.getText());
     ...
     * The initialiser already includes a semicolon from the inner rule so we strip it.
     */
    @Override
    ...
    public String visitForInit(translatorParser.ForInitContext ctx) {
        if (ctx.shortVarDecl() != null) return visit(ctx.shortVarDecl());
        return visit(ctx.assignment());
    }

    @Override
    public String visitForPost(translatorParser.ForPostContext ctx) {
        if (ctx.incDecStmt() != null) return visit(ctx.incDecStmt());
        return visit(ctx.assignment());
    }
     ...
     * The struct type name is found by walking up the parse tree to the enclosing function.
     */
    @Override
    public String visitReturnStmt(translatorParser.ReturnStmtContext ctx) {
        List<translatorParser.ExpressionContext> exprs = ctx.expression();
        if (exprs.isEmpty())  return "return;";
        if (exprs.size() == 1) return "return " + visit(exprs.get(0)) + ";";

        String funcName = enclosingFunctionName(ctx);
        List<String> vals = new ArrayList<>();
        for (var e : exprs) vals.add(visit(e));
    ...
    public String visitPrintStmt(translatorParser.PrintStmtContext ctx) {
        ...
        List<translatorParser.ExpressionContext> args =
            ctx.argumentList() != null ? ctx.argumentList().expression() : List.of();

        if (fn.equals("Printf")) {
            List<String> parts = new ArrayList<>();
            for (var a : args) parts.add(visit(a));
        ...
        if (newline) fmt.append("\\n");
        fmt.append("\"");

        if (rest.isEmpty()) return "printf(" + fmt + ");";
    ...
    public String visitArgumentList(translatorParser.ArgumentListContext ctx) {
        List<String> parts = new ArrayList<>();
        for (var e : ctx.expression()) parts.add(visit(e));
        return String.join(", ", parts);
    }
    ...
    public String visitMultiReturn(translatorParser.MultiReturnContext ctx) {
        ...
        for (var t : ctx.type_()) types.add(goToCType(t.getText()));
